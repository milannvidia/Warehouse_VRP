...
public class Warehouse {
    ...
    public void solve(boolean annotations) {
        ...
        for (Box box : this.boxes) {
            if (box.getPlaceLocation() == null) {
                box.setDigOutBox(true);
                continue;
            }
            if (box.getPlaceLocation() instanceof BoxStack) {
                ((BoxStack) box.getPlaceLocation()).addPlacesReserved();
                if (box.getCurrentLocation() instanceof BoxStack)
        ...
        // //TODO: can expand to digout even when no help needed but so that other
        // vehicle can reach more easily
        ...
        // stacks.sort(sorter);
        // for (BoxStack stack : stacks) {
        // if (stack.placesReserved + stack.boxes.size() + placesLeftVehicle >=
        // stack.heightLimit) continue;
        // if (stack.containsToMoveBox()) continue;
        // digoutCatch = stack;
        //
        ...
        // //find job
        // ArrayList<Box> toMove = new ArrayList<>(v.capacity);
        // out.sort((o1, o2) -> new SortByDistance(v.X, v.Y).compare(o1.placeLocation,
        // o2.placeLocation));
        // //first try to pick up difficult cases
        //
